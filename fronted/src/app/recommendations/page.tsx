...
export default function RecommendationsPage() {
  ...
  const analysis = useMemo(() => {
    ...
    const fleetUtilization =
      vehicleCount > 0
        ? (activeVehicles / vehicleCount) * 100
        : 0;
    ...
    if (totalProfit < 0) {
      addRecommendation(
        ...
        `Operating costs of ${money(
          totalOperatingCost,
        )} are higher than revenue of ${money(
          totalRevenue,
        )}.`,
    ...
    vehicleAnalysis
      .filter(
        (vehicle) =>
          vehicle.fuelCost >= 15000,
      )
      .sort(
        (a, b) =>
          b.fuelCost - a.fuelCost,
      )
      .slice(0, 3)
      .forEach((vehicle) => {
        addRecommendation(
          vehicle.fuelCost >= 25000
            ? "High"
            : "Medium",
          "Fuel Cost",
